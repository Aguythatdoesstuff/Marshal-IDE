...
namespace Compiler
{
    ...
    public abstract class BaseValidator
    {
        ...
        public List<ValidationWarning> Warnings { get; protected set; } = new List<ValidationWarning>();
        ...
        protected virtual void ValidateLines(List<string> lines, string filePath)
        {
            if (lines.Count == 0)
            {
                // Record a non-fatal warning so callers receive it in the structured report
                var fn = Path.GetFileName(filePath);
                Warnings.Add(new ValidationWarning(fn, 0, $"File is empty after sanitization: {filePath}"));
                try { Compiler.Logging.Logger.LogComponent(ComponentName, $"- [WARNING] File is empty after sanitization: {filePath}"); } catch { }
                return;
            }
        ...
        protected bool IsValidGfxName(string s, bool requiresPrefix = true, string fileName = null, int lineNumber = 0, string component = null)
        {
        ...
        protected bool IsValidId(string s, string fileName = null, int lineNumber = 0, string component = null, bool dotsAllowed = true)
        {
            component ??= "Validator";
            if (string.IsNullOrEmpty(s)) return false;
            ...
            // Allow both ASCII and Unicode letters for IDs. Optionally allow dots
            // as a separator character when dotsAllowed is true. We still warn above
            // for non-ASCII characters but accept them in the pattern so identifiers
            // containing characters like 'Ü' validate (with a warning).
            var pattern = dotsAllowed ? "^[\\p{L}0-9_\\.]+$" : "^[\\p{L}0-9_]+$";
        ...
        protected bool IsValidEventId(string s, string fileName, int lineNumber, string component)
        {
            ...
            // Allow both ASCII and Unicode letters for the id segment, followed by a dot and digits
            return Regex.IsMatch(s, "^[\\p{L}0-9_]+\\.[0-9]+$");
        }
